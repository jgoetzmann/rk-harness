...
push), `watcher.ps1` (live status window running `python -m rk_harness.watch`), `configure.py`
(edits config.json; `python configure.py explain`), and `config.example.json` (the defaults).
